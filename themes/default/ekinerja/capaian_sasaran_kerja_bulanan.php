...
<body>
	...
	?>
	<div class="header">
		PENILAIAN CAPAIAN SASARAN KERJA<br>PEGAWAI NEGERI SIPIL<br>BULAN <?= strtoupper(bulan($bulan))." TAHUN ".$tahun;?>
	</div>
	<div class="topright">
	...
	<table width="100%">
	    ...
	    <tr><td colspan="6" class="text-center"><strong>Penilaian Bulan <?=bulan($bulan)." Tahun ".$tahun;?></strong></td></tr>
	</table>
	  	...
	  	if(count($sql_kegiatan_tahunan) > 0){
	        ...
	        foreach ($sql_kegiatan_tahunan as $value) {
	        	$sql_realisasi = $this->db
	            	->select('b.angka_kredit, b.kuantitas, b.satuan, b.waktu ,b.periode_waktu, b.biaya, a.angka_kredit AS angka_kredit_rea, a.kuantitas AS kuantitas_rea, a.kualitas, a.waktu AS waktu_rea, a.biaya AS biaya_rea, a.perhitungan, a.nilai')
	            ...
	            if($sql_realisasi['biaya'] == 0)
	                ...
	                $biaya = number_format($sql_realisasi['biaya'],0,",",".");

	            if((int)$sql_realisasi['angka_kredit'] == 0)
	                $ak = '&nbsp;';
	            else
	                $ak = round($sql_realisasi['angka_kredit'], 1);

	            if($sql_realisasi['biaya_rea'] == 0)
	            	$biaya_realisasi = '&nbsp;';
	            else
	            	$biaya_realisasi =number_format($sql_realisasi['biaya_rea'],0,",",".");

	            if((int)$sql_realisasi['angka_kredit_rea'] == 0)
	                $ak_realisasi = '&nbsp;';
	            else
	                $ak_realisasi = round($sql_realisasi['angka_kredit_rea'], 1);
	            ...
	            echo '<tr>
	                ...
	                <td class="text-center">'.(int)$sql_realisasi['kuantitas'].' '.$sql_realisasi['satuan'].'</td>
	                <td class="text-center">100</td>
	                <td class="text-center">'.$sql_realisasi['waktu'].' '.$sql_realisasi['periode_waktu'].'</td>
	                <td class="text-center">'.$biaya.'</td>
	                <td class="text-center">'.$ak_realisasi.'</td>
	                <td class="text-center">'.(int)$sql_realisasi['kuantitas_rea'].' '.$sql_realisasi['satuan'].'</td>
	                <td class="text-center">'.$sql_realisasi['kualitas'].'</td>
	                <td class="text-center">'.$sql_realisasi['waktu_rea'].' '.$sql_realisasi['periode_waktu'].'</td>
	                <td class="text-center">'.$biaya_realisasi.'</td>
	                <td class="text-center">'.$sql_realisasi['perhitungan'].'</td>
